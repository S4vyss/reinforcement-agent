    ...
    with tf.device(self._device), tf.name_scope(self._scope):
      with tf.name_scope('get_next'):
        ...
        with tf.control_dependencies([assert_nonempty]):
          ...
          prio_all_ids = tf.map_fn(fn=lambda t: tf.add(t, batch_ids), elems=batch_offsets)
          prio_all_ids = tf.reshape(prio_all_ids, [-1])
          prio_all_ids = tf.math.mod(prio_all_ids, self._capacity) # because batch-ids are not capped, we need to do a mod here

          probabilities = self._prio_table.read(prio_all_ids)
          log_probabilities = tf.math.log([probabilities])
            
          ids_of_sampled_prio_ids = tf.random.categorical(log_probabilities, rows_shape[0])
          chosen_element_ids = tf.gather(prio_all_ids, ids_of_sampled_prio_ids)
          chosen_element_ids = tf.reshape(chosen_element_ids, [-1])
          chosen_element_probabilities = tf.gather(probabilities, ids_of_sampled_prio_ids)
